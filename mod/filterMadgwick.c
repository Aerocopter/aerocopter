...
/* ===== 默认参数 ===== */
#define betaDef     0.05f        /* 默认增益 */

/* ===== 内部状态 ===== */
static float beta;              /* 实际使用的增益 */
static float invSampleFreq;     /* 采样周期的倒数 (1/fs) */
static float q0, q1, q2, q3;    /* 四元数 */

/* ===== 快速逆平方根 (Quake III 算法，两次迭代提高精度) ===== */
static float invSqrt(float x)
{
    ...
    i = 0x5f3759df - (i >> 1);
    y = *(float*)&i;
    y = y * (1.5f - (halfx * y * y));   /* 第一次迭代 */
    y = y * (1.5f - (halfx * y * y));   /* 第二次迭代，提高精度 */
    return y;
}
...
/* ===== 获取四元数 ===== */
void Madgwick_GetQuaternion(float *q0_out, float *q1_out, float *q2_out, float *q3_out)
{
    if (q0_out) *q0_out = q0;
    if (q1_out) *q1_out = q1;
    if (q2_out) *q2_out = q2;
